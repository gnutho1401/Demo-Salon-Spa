  ...
  try {
    ...
    if (!customer_id) {
      ...
      if (!customer) {
        return error(
          res,
          "Không tìm thấy hồ sơ khách hàng liên kết với tài khoản này",
          404,
        );
      }
    ...
    const recommendations = await service.getStylistRecommendations(
      Number(customer_id),
      image_url,
      audience,
    );
    ...
    const clientError =
      /ảnh chưa chính diện|không nhận diện được khuôn mặt|vui lòng cung cấp image_url/i.test(
        err.message,
      );
...
async function getHistory(req, res) {
  try {
    ...
    const historyList = result.recordset
      .map((row) => {
        try {
