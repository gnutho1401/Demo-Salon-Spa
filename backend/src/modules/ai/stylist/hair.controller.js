...
async function tryOn(req, res) {
  try {
    ...
    if (!prompt && !styleId)
      return error(
        res,
        "Vui lòng chọn mẫu tóc hoặc nhập mô tả mong muốn.",
        400,
      );
    ...
    return success(
      res,
      result,
      "AI local đã áp dụng kiểu tóc thật lên ảnh",
      201,
    );
  } catch (err) {
    console.error("[AI Hair Try-on Error]:", err.message);
    const clientError =
      /^(Vui lòng|Ảnh chân dung|Chỉ hỗ trợ ảnh|Ảnh vượt quá|Ảnh đầu vào|URL ảnh không an toàn|Tài khoản chưa được liên kết|Mẫu tóc đã chọn)/i.test(
        err.message,
      );
...
async function getHistory(req, res) {
  try {
    return success(
      res,
      await service.getHistory(req.user.userId),
      "Lấy lịch sử thử tóc thành công",
    );
  } catch (err) {
...
async function getImage(req, res) {
  try {
    ...
    if (!["source", "result"].includes(variant))
      return error(res, "Loại ảnh không hợp lệ.", 400);
    ...
    return error(
      res,
      err.message,
      /Không tìm thấy|không có quyền/i.test(err.message) ? 404 : 500,
    );
  }
}
...
async function remove(req, res) {
  try {
    return success(
      res,
      await service.removeTryOn(req.user.userId, req.params.id),
      "Đã xóa ảnh thử tóc",
    );
  } catch (err) {
    return error(res, err.message, 404);
