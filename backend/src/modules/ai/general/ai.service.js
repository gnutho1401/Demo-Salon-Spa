...
- Xem lịch sử: Khách có thể xem tại WIDGET:SERVICE_HISTORY hoặc trang lịch sử.`;
    ...
    console.error('[AI Chat] Real AI generation failed:', err.message);
    throw new Error(`Hệ thống AI tạm thời gián đoạn kết nối: ${err.message}. Vui lòng thử lại sau giây lát!`);
  }
...
- reasoning phải ngắn gọn, đúng dữ liệu.`;
  ...
  try {
    const aiResponse = await generateContent(systemPrompt, JSON.stringify(customerData.slice(0, 15)), { jsonMode: true, maxTokens: 4000 });
    ...
    try {
      ...
      console.warn('[AI CRM] Retrying parse with repaired JSON...');
      let repaired = cleanAnswer.replace(/,\s*([\]}])/g, '$1');
      resultPayload = JSON.parse(repaired);
    }
    console.log('[AI] Successfully parsed AI JSON response for churn prediction');
  } catch (err) {
    console.error('[AI CRM] Real AI churn prediction failed:', err.message);
    throw new Error(`Dịch vụ CRM AI Churn Prediction không thể kết nối tới mô hình AI: ${err.message}. Vui lòng thử lại sau giây lát!`);
  }
  ...
  resultPayload.isFallback = isFallback;
