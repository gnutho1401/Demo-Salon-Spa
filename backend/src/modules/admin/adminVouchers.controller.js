const service = require("./adminVouchers.service");
...
async function create(req, res) {
  try {
    ...
    await logs.writeLog({
      ...
      actionName: "Admin create voucher",
      actionType: "CREATE",
      description: `Created voucher ${result?.Code || ""}`,
      ipAddress: req.ip,
      oldValue: null,
...
async function update(req, res) {
  try {
    ...
    await logs.writeLog({
      userId: userId(req),
      roleName: roleName(req),
      actionName: "Admin update voucher",
      actionType: "UPDATE",
      description: `Updated voucher ${result?.Code || before?.Code || ""}`,
      ipAddress: req.ip,
...
async function changeStatus(req, res) {
  try {
    ...
    await logs.writeLog({
      userId: userId(req),
      roleName: roleName(req),
      actionName: "Admin change voucher status",
      actionType: "STATUS",
      description: `Changed voucher status ${result?.Code || before?.Code || ""}`,
      ipAddress: req.ip,
...
async function disable(req, res) {
  try {
    const before = await service.getById(req.params.id);
    const result = await service.disable(req.params.id);

    await logs.writeLog({
      userId: userId(req),
      roleName: roleName(req),
      actionName: "Admin disable voucher",
      actionType: "STATUS",
      description: `Disabled voucher ${before?.Code || ""}`,
      ipAddress: req.ip,
      oldValue: JSON.stringify({ status: before?.Status }),
      newValue: JSON.stringify({ status: result?.Status }),
    });

    return success(res, result, "Disabled");
  } catch (err) {
    return error(res, err.message, 400);
  }
}
...
async function remove(req, res) {
  try {
    ...
    await logs.writeLog({
      userId: userId(req),
      roleName: roleName(req),
      actionName: "Admin delete voucher",
      actionType: "DELETE",
      description: `Deleted voucher ${before?.Code || ""}`,
      ipAddress: req.ip,
      oldValue: JSON.stringify(before),
      newValue: JSON.stringify(result),
    });
