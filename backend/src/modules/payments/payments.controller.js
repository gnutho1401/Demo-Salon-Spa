...
async function vnpayReturn(req, res) {
  try {
    const redirectUrl = await service.handleVnpayReturn(req.query);
    return res.redirect(redirectUrl);
  } catch (err) {
    return res.redirect(
      `${process.env.FRONTEND_URL || "http://localhost:5173"}/customer/payments?error=${encodeURIComponent(err.message)}`,
    );
  }
}
