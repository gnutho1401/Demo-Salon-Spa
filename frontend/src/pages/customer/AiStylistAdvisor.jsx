...
const AUDIENCES = [
  {
    value: "FEMALE",
    label: "Tóc nữ",
    description: "Bob, layer, shag, butterfly & màu hot",
  },
  {
    value: "MALE",
    label: "Tóc nam",
    description: "Crop, taper, curtain, mullet & texture",
  },
];
...
function Icon({ name, size = 18 }) {
  const paths = {
    camera: (
      <>
        <path d="M14.5 5 13 3H7L5.5 5H3a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2Z" />
    ...
    sparkle: (
      <>
        <path d="m12 3 1.2 3.4L16.5 8l-3.3 1.6L12 13l-1.2-3.4L7.5 8l3.3-1.6Z" />
        <path d="m5 13 .8 2.2L8 16l-2.2.8L5 19l-.8-2.2L2 16l2.2-.8Z" />
        <path d="m19 12 .6 1.4L21 14l-1.4.6L19 16l-.6-1.4L17 14l1.4-.6Z" />
      </>
    ),
    shield: (
      <>
...
function createLocalLookbookFallback(styles, audience) {
  ...
  const hairstyles = scopedStyles
    .filter((style) => style.type !== "COLOR")
    .slice(0, 2);
  const colors = scopedStyles
    .filter((style) => style.type === "COLOR")
    .slice(0, 2);
  ...
  return {
    analysis: {
      ...
      warnings: [
        "Không thể kết nối bộ phân tích; bạn vẫn có thể chọn mẫu tóc local.",
      ],
    },
    recommendations: {
...
function BeforeAfter({ source, result, value, onChange }) {
  return (
    <div
      className="stylist-compare"
      aria-label="So sánh ảnh gốc và ảnh thử tóc"
    >
      <img src={source} alt="Ảnh gốc trước khi thử tóc" />
      <div
        className="stylist-compare-after"
        style={{ clipPath: `inset(0 0 0 ${value}%)` }}
      >
        <img src={result} alt="Ảnh sau khi AI áp dụng kiểu tóc" />
      </div>
      <div
        className="stylist-compare-line"
        style={{ left: `${value}%` }}
        aria-hidden="true"
      >
        <span>↔</span>
...
export default function AiStylistAdvisor() {
  ...
  useEffect(() => {
    ...
    video
      ...
      .catch(() => {
        if (active)
          setError(
            "Camera đã mở nhưng chưa phát được hình ảnh. Hãy thử đóng và mở lại camera.",
          );
      });
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (stylesResult.status === "fulfilled") {
        ...
        setStyleError(
          stylesResult.reason?.response?.data?.message ||
            "Không thể tải danh mục mẫu tóc.",
        );
      }
      if (historyResult.status === "fulfilled")
        setHistory(extractApiData(historyResult.value) || []);
    });
    return () => {
      active = false;
      streamRef.current?.getTracks().forEach((track) => track.stop());
  ...
  const capturePhoto = () => {
    ...
    if (!cameraReady || !video?.videoWidth) {
      setError(
        "Camera chưa sẵn sàng. Vui lòng chờ hình ảnh xuất hiện rồi chụp lại.",
      );
      return;
    }
    const maxSide = 1600;
  ...
  const analyzePhoto = async () => {
    if (!photo)
      return setError("Vui lòng chụp hoặc tải lên một ảnh chân dung.");
    if (!consent)
      return setError("Bạn cần đồng ý xử lý ảnh trước khi sử dụng AI Stylist.");
    setAnalysisLoading(true);
    setError("");
    try {
      const response = await axiosClient.post("/ai/stylist/analyze", {
        image_url: photo,
        audience,
      });
      ...
      if (styles.length > 0) {
        ...
        setError(
          "Phân tích ảnh đang gián đoạn; hệ thống đã mở lookbook local để bạn vẫn có thể thử tóc.",
        );
      } else {
        setError(
          requestError.response?.data?.message ||
            requestError.message ||
            "AI không thể phân tích ảnh này.",
        );
      }
    } finally {
  ...
  const createTryOn = async () => {
    if (!photo || !analysis)
      return setError("Hãy hoàn tất bước phân tích chân dung trước.");
    if (!selectedStyle && !customPrompt.trim())
      return setError("Hãy chọn một mẫu tóc hoặc nhập mô tả riêng.");
  ...
  const openHistoryItem = async (item) => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ||
          "Không thể mở ảnh trong lịch sử.",
      );
    } finally {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      await axiosClient.patch(
        `/ai/stylist/tryon/${activeResult.try_on_id}/favorite`,
        { is_favorite: nextValue },
      );
      setActiveResult((item) => ({ ...item, is_favorite: nextValue }));
      setHistory((items) =>
        items.map((item) =>
          item.try_on_id === activeResult.try_on_id
      ...
      setError(
        requestError.response?.data?.message ||
          "Không thể cập nhật mẫu yêu thích.",
      );
    }
  };
  ...
  const deleteResult = async () => {
    ...
    try {
      ...
      setHistory((items) =>
        items.filter((item) => item.try_on_id !== activeResult.try_on_id),
      );
      ...
      setError(
        requestError.response?.data?.message || "Không thể xóa ảnh thử tóc.",
      );
    }
  };
  ...
  const downloadResult = async () => {
    ...
    const blob = await fetch(activeResult.resultUrl).then((response) =>
      response.blob(),
    );
  ...
  const chooseRecommendation = (item) => {
    const catalogStyle = styles.find(
      (style) => Number(style.style_id) === Number(item.style_id),
    );
  ...
  const bookingServiceId =
    activeResult?.style?.service_id ||
    selectedStyle?.service_id ||
    analysis?.booking_suggestion?.recommended_service_id ||
    "";
  const bookingEmployeeId =
    analysis?.booking_suggestion?.suggested_stylist_id || "";
    ...
    <CustomerLayout>
      <div className="ai-stylist-page">
        <header className="ai-stylist-hero">
          <div>
            <span className="ai-stylist-eyebrow">
              AI Stylist · Gương thử tóc
            </span>
            <h1>
              Tìm mái tóc hợp với bạn,
              <br />
              trước khi ngồi vào ghế salon.
            </h1>
            <p>
              AI phân tích ảnh thật, gợi ý từ dữ liệu dịch vụ của salon và áp
              dụng trực tiếp từng kiểu tóc lên chính khuôn mặt của bạn.
            </p>
          </div>
          <div className="ai-stylist-trust">
            <Icon name="shield" size={20} />
            <div>
              <strong>Ảnh được bảo vệ riêng tư</strong>
              <span>
                Không lưu base64 trong database · Có thể xóa bất kỳ lúc nào
              </span>
            </div>
          </div>
        </header>
        ...
        <div
          className="ai-audience-selector"
          role="group"
          aria-label="Chọn thư viện kiểu tóc nam hoặc nữ"
        >
          <div>
            <span>Lookbook cá nhân</span>
            <strong>Bạn muốn khám phá kiểu tóc nào?</strong>
          </div>
          {AUDIENCES.map((item) => (
            <button
              type="button"
        ...
        <ol className="ai-stylist-steps" aria-label="Tiến trình thử tóc">
          {["Ảnh chân dung", "Tư vấn kiểu tóc", "Gương thử tóc"].map(
            (label, index) => (
              <li
                key={label}
                className={
        ...
        <section className="ai-stylist-workspace">
          <div className="ai-stylist-mirror-panel">
            <div className="ai-stylist-panel-heading">
              ...
              {activeResult && (
                <span className="ai-real-badge">
                  <i /> AI ảnh thật · {activeResult.provider}
                </span>
              )}
            </div>

            <div
              className={`ai-stylist-mirror ${photo || cameraActive ? "has-photo" : ""}`}
            >
              {cameraActive ? (
                <div className="ai-stylist-camera-view">
                  ...
                  <button
                    type="button"
                    className="ai-camera-shutter"
                    onClick={capturePhoto}
                    disabled={!cameraReady}
                    aria-label={
                      cameraReady ? "Chụp ảnh" : "Camera đang khởi động"
                    }
                  >
                    <span />
                  </button>
                ...
                <BeforeAfter
                  source={activeResult.sourceUrl || photo}
                  result={activeResult.resultUrl}
                  value={compareValue}
                  onChange={setCompareValue}
                />
              ) : photo ? (
                <img
                  className="ai-stylist-portrait"
                  src={photo}
                  alt="Ảnh chân dung đã chọn"
                />
              ) : (
                <button
                  type="button"
                  className="ai-stylist-empty-mirror"
                  onClick={() => fileInputRef.current?.click()}
                >
                  <span className="ai-stylist-empty-icon">
                    <Icon name="image" size={28} />
                  </span>
                  <strong>Bắt đầu với một ảnh rõ khuôn mặt</strong>
                  <small>
                    Ánh sáng đều · nhìn thẳng · không đội mũ · tối đa 10 MB
                  </small>
                  <em>
                    <Icon name="upload" size={16} /> Chọn ảnh từ thiết bị
                  </em>
                </button>
              )}
              ...
              {tryOnLoading && (
                <div
                  className="ai-stylist-generating"
                  role="status"
                  aria-live="polite"
                >
                  <div className="ai-hair-strands" aria-hidden="true">
                    <i />
                    <i />
                    <i />
                  </div>
                  ...
                  <div className="ai-generation-progress">
                    <i style={{ width: `${24 + loadingStage * 22}%` }} />
                  </div>
                  <small>
                    Thường mất khoảng 15–45 giây. Vui lòng giữ trang này mở.
                  </small>
                </div>
              )}
            </div>
            ...
            <div className="ai-stylist-mirror-actions">
              ...
              >
                <Icon name="camera" />{" "}
                {cameraActive ? "Đóng camera" : "Chụp ảnh"}
              </button>
              {photo && (
                <button
                  type="button"
            ...
            {activeResult && (
              <div className="ai-stylist-result-actions">
                <button
                  type="button"
                  className={activeResult.is_favorite ? "is-favorite" : ""}
                  onClick={toggleFavorite}
                >
                  <Icon name="heart" />{" "}
                  {activeResult.is_favorite ? "Đã lưu" : "Lưu mẫu"}
                </button>
                <button type="button" onClick={downloadResult}>
                  <Icon name="download" /> Tải ảnh
                </button>
                <Link to={bookingUrl}>
                  <Icon name="calendar" /> Đặt lịch kiểu này
                </Link>
                <button
                  type="button"
                ...
                >
                  <Icon name="trash" />{" "}
                  {deleteArmed ? "Xác nhận xóa" : "Xóa ảnh"}
                </button>
              </div>
            )}
          </div>
          ...
          <aside className="ai-stylist-consult-panel">
            {!analysis ? (
              <>
                <div className="ai-consult-heading">
                  <span>01</span>
                  <div>
                    <strong>Đọc đặc điểm tóc & khuôn mặt</strong>
                    <p>
                      AI dùng ảnh thật và lịch sử dịch vụ của bạn để tạo tư vấn
                      cá nhân hóa.
                    </p>
                  </div>
                </div>
                <div className="ai-photo-guide">
                  <div>
                    <i className="is-good" />
                    <span>
                      <strong>Ảnh nên dùng</strong>Mặt nhìn thẳng, đủ sáng, thấy
                      rõ toàn bộ tóc.
                    </span>
                  </div>
                  <div>
                    <i className="is-bad" />
                    <span>
                      <strong>Nên tránh</strong>Mũ, kính che mặt, ảnh mờ hoặc
                      nhiều người.
                    </span>
                  </div>
                </div>
                <label className="ai-consent-row">
                  ...
                  <span>
                    <b>
                      Tôi đồng ý cho hệ thống xử lý ảnh để tư vấn và thử tóc.
                    </b>
                    <small>
                      Ảnh chỉ hiển thị cho tài khoản này và tự hết hạn theo
                      chính sách lưu trữ.
                    </small>
                  </span>
                </label>
              ...
              <>
                <div className="ai-consult-heading">
                  <span>02</span>
                  <div>
                    <strong>Hồ sơ phong cách của bạn</strong>
                    <p>
                      {analysis.api_enhanced
                        ? "API phân tích + local kiểm tra an toàn"
                        : "Local fallback"}{" "}
                      · Ảnh thử tóc luôn tạo local.
                    </p>
                  </div>
                </div>
                {analysis.fallback_used && (
                  <div className="ai-local-mode-note" role="status">
                    ...
                    <span>
                      <strong>Đang dùng chế độ local</strong> API nâng cao không
                      khả dụng nhưng lookbook và thử tóc vẫn hoạt động.
                    </span>
                  </div>
                )}
                <dl className="ai-profile-facts">
                  <div>
                    <dt>Dáng khuôn mặt</dt>
                    <dd>{analysis.analysis?.face_shape || "Đã phân tích"}</dd>
                  </div>
                  <div>
                    <dt>Chất tóc hiện tại</dt>
                    <dd>{analysis.analysis?.hair_type || "Đã phân tích"}</dd>
                  </div>
                  <div>
                    <dt>Tông da</dt>
                    <dd>{analysis.analysis?.skin_tone || "Đã phân tích"}</dd>
                  </div>
                </dl>
                ...
                >
                  <Icon name="reset" /> Phân tích lại ảnh
                </button>
              </>
            )}
        ...
        {analysis && (
          <section className="ai-lookbook-section">
            <div className="ai-section-heading">
              <div>
                <span>02 · Lookbook salon</span>
                <h2>Chọn một mẫu để thử trực tiếp</h2>
                <p>
                  Mỗi lựa chọn tạo một ảnh mới; bạn có thể so sánh nhiều mẫu
                  trong cùng phiên.
                </p>
              </div>
              <div
                className="ai-lookbook-filters"
                role="group"
                aria-label="Lọc mẫu tóc"
              >
                {FILTERS.map((filter) => (
                  <button
                    type="button"
            ...
            {styleError ? (
              <div className="ai-stylist-error">
              ...
              <div className="ai-style-grid">
                {filteredStyles.map((style) => (
                  <button
                    type="button"
                    key={style.style_id}
                    className={`ai-style-card ${selectedStyle?.style_id === style.style_id ? "is-selected" : ""}`}
                    onClick={() => chooseStyle(style)}
                  >
                    <div className="ai-style-image">
                      <img
                        src={style.thumbnail_url}
                        alt={`Mẫu ${style.name}`}
                      />
                      {style.is_trending && (
                        <b className="ai-trend-badge">
                          Hot {style.trend_year || 2026}
                        </b>
                      )}
                      <span
                        style={{ background: style.accent_color || "#d6b57e" }}
                      />
                    </div>
                    <div className="ai-style-card-body">
                      <small>
                        {style.type === "COLOR"
                          ? "Màu tóc"
                          : style.length === "SHORT"
                      ...
                      <em>
                        {MAINTENANCE_LABELS[style.maintenance] ||
                          "Chăm sóc vừa"}
                      </em>
                    </div>
            ...
            <div className="ai-custom-look">
              <div>
                <span>Tinh chỉnh theo ý bạn</span>
                <strong>
                  {selectedStyle
                    ? `Đang chọn: ${selectedStyle.name}`
                    : "Mô tả mái tóc bạn muốn"}
                </strong>
              </div>
              <textarea
                value={customPrompt}
                ...
                maxLength={800}
                placeholder="Ví dụ: giữ phom layer này nhưng đổi sang màu nâu hạt dẻ, mái bay nhẹ và độ dài ngang vai…"
              />
              <button
                type="button"
                onClick={createTryOn}
                disabled={
                  tryOnLoading || (!selectedStyle && !customPrompt.trim())
                }
              >
                <Icon name="sparkle" />{" "}
                {tryOnLoading
        ...
        {(sessionResults.length > 0 || history.length > 0) && (
          <section className="ai-history-section">
            <div className="ai-section-heading">
              <div>
                <span>03 · Bộ sưu tập riêng</span>
                <h2>Những kiểu bạn đã thử</h2>
                <p>
                  Kết quả chỉ tải qua phiên đăng nhập của bạn. Chọn một mẫu để
                  mở lại trong gương.
                </p>
              </div>
            ...
            <div
              className={`ai-history-rail ${historyLoading ? "is-loading" : ""}`}
            >
              {history
                .filter((item) => item.status === "SUCCEEDED")
                .map((item) => {
                  const liveResult = sessionResults.find(
                    (result) => result.try_on_id === item.try_on_id,
                  );
                  return (
                    <button
                      type="button"
                      key={item.try_on_id}
                      className={
                        activeResult?.try_on_id === item.try_on_id
                          ? "is-active"
                          : ""
                      }
                      onClick={() =>
                        liveResult
                          ? setActiveResult(liveResult)
                          : openHistoryItem(item)
                      }
                    >
                      <div>
                        {liveResult?.resultUrl ? (
                          <img
                            src={liveResult.resultUrl}
                            alt="Kết quả thử tóc"
                          />
                        ) : (
                          <span
                            style={{
                              background: item.style?.accent_color || "#d6b57e",
                            }}
                          >
                            <Icon name="image" />
                          </span>
                      ...
                      <small>
                        ...
                        ·{" "}
                        {Math.max(1, Math.round((item.latency_ms || 0) / 1000))}
                        s
                      </small>
                      {item.is_favorite && (
                        <i>
        ...
        <footer className="ai-stylist-disclaimer">
          ...
          <p>
            <strong>Kết quả dùng để tham khảo phong cách.</strong> Màu và phom
            tóc thực tế còn phụ thuộc nền tóc, chất tóc và kỹ thuật thực hiện.
            Hãy đặt lịch để stylist kiểm tra trực tiếp trước khi dùng hóa chất.
          </p>
        </footer>
      </div>
    </CustomerLayout>
