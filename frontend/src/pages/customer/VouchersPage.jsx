...
export default function VouchersPage() {
  ...
  return <CustomerLayout>
    <div className="section-head" style={{ marginBottom: '28px' }}>
      <div>
        <div className="eyebrow" style={{ color: '#e96a95', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>Voucher</div>
        <h2 className="section-title" style={{ fontSize: '32px', color: '#2d1424', fontWeight: 800, margin: '6px 0 0 0' }}>Ví Voucher của tôi</h2>
      </div>
    </div>
    ...
    {loading ? (
      <p style={{ textAlign: 'center', color: '#8c7784', fontSize: '16px', padding: '40px 0' }}>Đang tải danh sách voucher...</p>
    ) : (
      <>
        <h3 style={{ fontSize: '20px', color: '#2d1424', fontWeight: 800, marginBottom: '20px' }}>Voucher đã lưu</h3>
        <div className="voucher-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', marginBottom: '40px' }}>
          {mine.length ? mine.map((v) => {
            ...
            const discountNumStr = isPercent ? String(v.DiscountValue) : money(v.DiscountValue);
            
            return (
              <div className={`luxury-voucher-card-wrapper ${isFullyUsed ? 'used' : ''}`} key={v.VoucherId} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <article className="luxury-voucher-card" style={{ opacity: isFullyUsed ? 0.65 : 1 }}>
                  ...
                  <div className="voucher-right">
                    <div className="voucher-right-top">
                      ...
                      <p className="voucher-terms">
                        Điều kiện: {!v.MinOrderAmount || Number(v.MinOrderAmount) === 0 ? "Mọi đơn hàng" : `Đơn từ ${money(v.MinOrderAmount)}đ`} 
                        {v.MaxDiscountAmount > 0 && ` | Giảm tối đa ${money(v.MaxDiscountAmount)}đ`}
                      </p>
                      <p className="voucher-expiry">Hạn dùng: {date(v.EndDate)}</p>
                      <p style={{ fontWeight: 'bold', color: isFullyUsed ? '#ef4444' : '#10b981', marginTop: '6px', fontSize: '12px' }}>
                        Lượt dùng còn lại: {isFullyUsed ? 'Hết lượt' : `${personalRemaining}/1 lần`}
                      </p>
                    </div>

                    <div className="voucher-right-bottom">
                      {isFullyUsed ? (
                        <button className="btn-voucher-action" disabled style={{ background: '#e9ecef', color: '#adb5bd', cursor: 'not-allowed', boxShadow: 'none' }}>
                          Đã dùng
                        </button>
                      ) : (
                        <button className="btn-voucher-action" onClick={() => navigate('/customer/booking')}>
                          Dùng ngay
                        </button>
                      )}
                    </div>
                ...
                {((v.Usages && v.Usages.length > 0) || unpaidUsages.length > 0) && (
                  <div 
                  ...
                  >
                    {v.Usages && v.Usages.length > 0 && (
                      <div className="usage-history">
                        <b style={{ color: '#2d1424', display: 'block', marginBottom: '4px' }}>Lịch sử sử dụng:</b>
                        <ul style={{ margin: 0, paddingLeft: '16px', color: '#7b6874' }}>
                          {v.Usages.map(usage => (
                            <li key={usage.AppointmentId} style={{ marginBottom: '2px' }}>
            ...
            <div className="dashboard-card" style={{ gridColumn: '1 / -1', background: '#fff9fa', border: '1px dashed rgba(239, 79, 131, 0.2)', borderRadius: '24px', padding: '36px', textAlign: 'center' }}>
              <p className="muted" style={{ margin: 0, color: '#8c7784', fontSize: '15px' }}>Bạn chưa lưu voucher nào. Hãy thu thập voucher phía dưới nhé! 🌸</p>
            </div>
          )}
        </div>

        <h3 style={{ fontSize: '20px', color: '#2d1424', fontWeight: 800, marginBottom: '20px', marginTop: '36px' }}>Voucher đang có</h3>
        <div className="voucher-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px' }}>
          {all.map((v) => {
              ...
              <article className="luxury-voucher-card" key={v.VoucherId}>
                ...
                <div className="voucher-right">
                  <div className="voucher-right-top">
                    ...
                    <p className="voucher-expiry">Hạn dùng: {date(v.EndDate)}</p>
                    <p style={{ fontSize: '12px', color: '#8c7784', marginTop: '6px' }}>Số lượng còn lại: {v.Quantity}</p>
                  </div>
                  ...
                  <div className="voucher-right-bottom">
                    {isSaved ? (
                      ...
                      <button className="btn-voucher-action" onClick={() => save(v.VoucherId)}>
                        Lưu voucher
                      </button>
                    )}
                  </div>
