...
function formatDate(dateValue) {
  if (!dateValue) return "—";
  let str = dateValue instanceof Date ? dateValue.toISOString().slice(0, 10) : String(dateValue);
  if (str.includes("T")) {
    str = str.split("T")[0];
  }
...
function PackageDetailPanel({ pkg, onClose, onRefresh, onCancelCombo }) {
  ...
  const openTechModalForPackageStep = async (appointmentId, stepId) => {
    ...
    try {
      const res = await axiosClient.get(`/appointments/${appointmentId}/available-technicians${stepId ? `?appointmentServiceId=${stepId}` : ""}`);
      ...
      setTechError(err.response?.data?.message || "Không lấy được danh sách KTV rảnh trong giờ");
      setTechList([]);
    } finally {
  ...
  const handleConfirmChangeTech = async (e) => {
    ...
    try {
      ...
      await axiosClient.patch(`/appointments/${apptId}/change-technician`, {
        ...
        appointmentServiceId: selectedTechStepId ? Number(selectedTechStepId) : null
      });
    ...
    <div className="pkg-detail-panel" style={{ background: "#fff", borderRadius: 24, padding: 28, border: "1.5px solid #fbcfe8", boxShadow: "0 10px 30px rgba(236,72,153,0.08)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: "Ngày bắt đầu", value: formatDate(d.StartDate || d.PaidAt || d.CreatedAt) },
      ...
      {d.ActiveAppointment && (() => {
        const apptStat = String(d.ActiveAppointment.Status || "").toUpperCase();
        ...
        return (
          <div style={{
            ...
            padding: 20,
            marginBottom: 24,
            boxShadow: isInProgress ? "0 8px 24px rgba(16, 185, 129, 0.15)" : "none"
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
              <div>
                <span style={{
                  background: isInProgress ? "#059669" : isCheckedIn ? "#2563eb" : "#db2777",
                  color: "#fff",
                  fontSize: 10,
                  fontWeight: 800,
                  padding: "3px 10px",
                  ...
                  {isInProgress ? "▶ ĐANG THỰC HIỆN LIỆU TRÌNH TẠI SALON" : isCheckedIn ? "◇ ĐÃ CHECK-IN TẠI SALON" : "📅 ĐÃ CÓ LỊCH HẸN COMBO SẮP TỚI"}
                </span>
                <h4 style={{ margin: "8px 0 4px 0", fontSize: 16, fontWeight: 800, color: isInProgress ? "#065f46" : isCheckedIn ? "#1e3a8a" : "#831843" }}>
                ...
                <span style={{ fontSize: 13, color: isInProgress ? "#047857" : isCheckedIn ? "#1d4ed8" : "#9d174d", fontWeight: "600" }}>
                  Trạng thái ca: <b>{isInProgress ? "▶ Đang thực hiện" : isCheckedIn ? "◇ Đã check-in" : "✓ Đã xác nhận"}</b> • Trưởng nhóm KTV: <b>{d.ActiveAppointment.PrimaryTechName || "Tự động phân công"}</b>
                </span>

                {/* STEP-BY-STEP SERVICE & TECHNICIAN BREAKDOWN */}
                {d.ActiveAppointment.Services && d.ActiveAppointment.Services.length > 0 && (
                  <div style={{ marginTop: 14, borderTop: isInProgress ? "1px dashed #a7f3d0" : "1px dashed #bfdbfe", paddingTop: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 800, color: isInProgress ? "#065f46" : "#1e40af", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>
                      ✂️ TIẾN TRÌNH THỰC HIỆN TỪNG DỊCH VỤ TRONG COMBO:
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
                      {d.ActiveAppointment.Services.map((svcStep, idx) => {
                        const stepStat = String(svcStep.StepStatus || "PENDING").toUpperCase();
                        ...
                        return (
                          <div key={svcStep.AppointmentServiceId || idx} style={{
                            ...
                            padding: "10px 14px",
                            borderRadius: 12,
                            border: isStepDoing ? "2px solid #f59e0b" : isStepDone ? "1.5px solid #86efac" : "1px solid #e2e8f0",
                            boxShadow: isStepDoing ? "0 2px 10px rgba(245, 158, 11, 0.2)" : "none"
                          }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                              <b style={{ color: "#1e293b", fontSize: 13 }}>{idx + 1}. {svcStep.ServiceName}</b>
                              <span style={{
                                background: isStepDone ? "#dcfce7" : isStepDoing ? "#fef3c7" : "#f1f5f9",
                                color: isStepDone ? "#15803d" : isStepDoing ? "#b45309" : "#64748b",
                                fontSize: 10,
                                fontWeight: 800,
                                padding: "2px 8px",
                                borderRadius: 10
                              }}>
                                {isStepDone ? "✓ Hoàn thành" : isStepDoing ? "▶ Đang làm" : "○ Chờ làm"}
                              </span>
                            </div>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
                              ...
                              <div
                                ...
                                onClick={() => !isLive && openTechModalForPackageStep(d.ActiveAppointment.AppointmentId, svcStep.AppointmentServiceId)}
                                title={isLive ? "Không thể đổi KTV khi ca hẹn đã check-in/đang làm" : "Nhấn vào để đổi KTV mới"}
                              >
                                <img
                                  src={resolveFileUrl(svcStep.TechnicianAvatar) || "/images/avatars/default-avatar.png"}
                                  alt={svcStep.TechnicianName}
                                  style={{ width: 22, height: 22, borderRadius: "50%", objectFit: "cover", border: "1.5px solid #2563eb" }}
                                />
                                <b style={{ fontSize: 11, color: "#334155" }}>{svcStep.TechnicianName || "KTV Rảnh"} {!isLive && "✎"}</b>
                              </div>
                            </div>
                          </div>
                        );
            ...
            {isLive ? (
              <div style={{
                ...
                border: isInProgress ? "1.5px solid #a7f3d0" : "1.5px solid #fde68a",
                borderRadius: 12,
                padding: "10px 14px",
                fontSize: 12,
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
                ...
                <button
                  ...
                  onClick={() => {
                    if (onCancelCombo) {
                      onCancelCombo(d.CustomerPackageId || pkg.CustomerPackageId, d.ActiveAppointment?.AppointmentId);
                    }
                  }}
                >
                  ❌ Hủy Lịch Hẹn Combo
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {usages.data.map((u, idx) => (
              <div key={u.UsageId || idx} style={{ background: "#f8fafc", padding: "12px 16px", borderRadius: 12, border: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div>
                  <b style={{ fontSize: 13, color: "#1e293b", display: "block" }}>{formatDateTime(u.UsedDate || u.AppointmentDate || u.UsedAt, u.StartTime)}</b>
                  <span style={{ fontSize: 12, color: "#64748b" }}>{u.ServiceName || "Dịch vụ combo"} · KTV: {u.EmployeeName || u.TechnicianName || "KTV Salon"}</span>
                </div>
      ...
      <Modal open={showBookModal} onClose={() => setShowBookModal(false)} title={isRescheduleMode ? "🔄 Đổi Lịch Hẹn Combo" : "✨ Đặt Lịch Hẹn Sử Dụng Combo"}>
        {bookingSuccess ? (
          <div style={{ textAlign: "center", padding: "10px 0" }}>
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", background: "#dcfce7", color: "#15803d", border: "1px solid #bbf7d0", padding: "6px 16px", borderRadius: "20px", fontWeight: "700", fontSize: "0.85rem", margin: "4px 0 12px 0" }}>
          ...
          <div className="pkg-form">
            ...
            <label style={{ display: "block", marginBottom: 16 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "#334155" }}>📝 Ghi chú dặn dò (Không bắt buộc):</span>
              <textarea
                value={bookingForm.notes || ""}
                onChange={(e) => setBookingForm({ ...bookingForm, notes: e.target.value })}
                placeholder="Ví dụ: Da nhạy cảm, yêu cầu chỗ đỗ xe, có tiền sử dị ứng..."
                rows={3}
                style={{ width: "100%", padding: 10, marginTop: 4, borderRadius: 8, border: "1px solid #cbd5e1", fontSize: 13, fontFamily: "inherit", resize: "vertical" }}
              />
            </label>
      ...
      <Modal open={showTechModal} onClose={() => setShowTechModal(false)} title="👤 Chọn Kỹ Thuật Viên Rảnh Trong Giờ">

        <form onSubmit={handleConfirmChangeTech}>
          {techError && <div className="alert error" style={{ marginBottom: 12 }}>{techError}</div>}
          ...
          {loadingTechs ? (
            ...
            <div className="alert error" style={{ marginBottom: 12 }}>
              Không có Kỹ thuật viên rảnh phù hợp cho khung giờ này. Vui lòng liên hệ Lễ tân để hỗ trợ.
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 320, overflowY: "auto", margin: "12px 0" }}>
              {techList.map((t) => (
                <label
                  key={t.EmployeeId}
                  style={{
                    ...
                    padding: "10px 14px",
                    borderRadius: 12,
                    border: String(selectedNewTechId) === String(t.EmployeeId) ? "2px solid #ec4899" : "1px solid #cbd5e1",
                    background: String(selectedNewTechId) === String(t.EmployeeId) ? "#fdf2f8" : "#ffffff",
                    cursor: "pointer"
                  }}
                >
                  <input
                  ...
                  />
                  <img
                    src={resolveFileUrl(t.AvatarUrl || t.ImageUrl) || "/images/avatars/default-avatar.png"}
                    alt={t.FullName}
                    style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover" }}
                  />
                  <div style={{ flex: 1 }}>
                    <b style={{ fontSize: 13, color: "#1e293b", display: "block" }}>
                      {t.FullName} {t.IsCurrent ? <span style={{ color: "#ec4899", fontSize: 11 }}>(Hiện tại)</span> : ""}
                    </b>
                    <span style={{ fontSize: 11, color: "#64748b" }}>{t.Specialization || t.Position || "Chuyên gia làm đẹp"}</span>
                  </div>
                </label>
              ))}
            </div>
...
export default function CustomerPackages() {
  ...
  const [cancelReasonInput, setCancelReasonInput] = useState("Thay đổi kế hoạch cá nhân");
  ...
  const confirmCancelCombo = async () => {
    ...
    try {
      const res = await axiosClient.post(`/packages/my/${cancelModalPkgId}/cancel-appointment`, {
        reason: cancelReasonInput,
        appointmentId: cancelModalApptId
      });
      const successMsg = res.data?.message || res.data?.data?.message || "Đã hủy lịch hẹn Combo thành công!";
      alert("🎉 " + successMsg);
      ...
      if (selectedPkg) {
        setSelectedPkg(prev => prev ? { ...prev, _refresh: Date.now() } : null);
      }
    } catch (err) {
      alert(err.response?.data?.message || "Không thể hủy lịch hẹn Combo. Vui lòng thử lại!");
    } finally {
  ...
  useEffect(() => {
    ...
    const targetPkgId = params.get("packageId") || params.get("customerPackageId");
    const isPaid = params.get("paid") === "1" || params.get("status") === "success";
    ...
    if (targetPkgId) {
      ...
      axiosClient.get(`/packages/my/${targetPkgId}/detail`)
        .then(res => {
        ...
        .catch(() => {
          axiosClient.get("/packages/my").then(r => {
            ...
            const found = list.find(p => String(p.CustomerPackageId) === String(targetPkgId));
  ...
  const openReviewModal = (appt) => {
    if (appt && appt.AppointmentId) {
      navigate(`/customer/reviews?type=combo&appointmentId=${appt.AppointmentId}`);
    } else {
      navigate("/customer/reviews?type=combo");
    }
  };
  ...
  const handleCancelComboAppointment = async (customerPackageId) => {
    ...
    try {
      setLoading(true);
      const res = await axiosClient.post(`/packages/my/${customerPackageId}/cancel-appointment`, { reason });
      ...
      <section className="pkg-container" style={{ maxWidth: 1200, margin: "0 auto", padding: "10px 20px 60px 20px" }}>
                ...
                <div className="my-combo-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 20 }}>
                  {mine.length ? (
                    mine.map((p) => {
                      ...
                      const relatedHistory = comboHistory.find(h => Number(h.CustomerPackageId) === Number(p.CustomerPackageId));
                      const isPkgReviewed = relatedHistory ? relatedHistory.IsReviewed : false;
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                {p.Status === "PENDING_PAYMENT" ? (
                                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                                    <button
                                      type="button"
                                      onClick={(e) => repayVnpay(e, p.CustomerPackageId)}
                                      style={{
                                    ...
                                    <button
                                      type="button"
                                      onClick={(e) => repayPayos(e, p.CustomerPackageId)}
                                      style={{
                                ...
                                ) : (p.Status === "COMPLETED" || p.Status === "USED_UP") ? (
                                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                                    <button
                                      type="button"
                                      onClick={(e) => { e.stopPropagation(); setCustomerActiveTab("history"); }}
                                      style={{ background: "#fdf2f8", border: "1px solid #fbcfe8", color: "#db2777", fontSize: 11, fontWeight: 700, borderRadius: 10, padding: "4px 8px", cursor: "pointer" }}
                                    >
                                      📜 Lịch sử
                                    </button>
                                    {isPkgReviewed ? (
                                      <span style={{ fontSize: 11, fontWeight: 700, color: "#166534", background: "#f0fdf4", border: "1px solid #bbf7d0", padding: "4px 8px", borderRadius: 10 }}>
                                        ✅ Đã đánh giá
                                      </span>
                                    ) : (
                                      <button
                                        type="button"
                                        onClick={(e) => { e.stopPropagation(); navigate("/customer/reviews?type=combo"); }}
                                        style={{ background: "#ec4899", border: "none", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: 10, padding: "4px 8px", cursor: "pointer" }}
                                      >
                                        ⭐ Đánh giá
                                      </button>
                                    )}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                          <span style={{ background: "linear-gradient(135deg, #ec4899 0%, #db2777 100%)", color: "#fff", fontSize: 11, fontWeight: 800, padding: "3px 10px", borderRadius: 10 }}>
                          ...
                          {item.StartTime && (
                            <span style={{ fontSize: 12, color: "#db2777", fontWeight: 700, background: "#fdf2f8", padding: "2px 8px", borderRadius: 8, border: "1px solid #fbcfe8" }}>
                              ⏰ {item.StartTime?.slice(0,5)}{item.EndTime ? ` - ${item.EndTime?.slice(0,5)}` : ""}
                            </span>
                          )}
                        </div>
                        <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#831843" }}>{item.PackageName}</h3>
                      </div>

                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <StatusBadge status={item.Status} />
                        {["CONFIRMED", "BOOKED", "PENDING"].includes(item.Status) && item.CustomerPackageId && (
                          <button
                            onClick={() => triggerCancelCombo(item.CustomerPackageId, item.AppointmentId)}
                            style={{
                              background: "#fff1f2",
        ...
        {customerActiveTab === "reviews" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              ...
              <span style={{ fontSize: 12, color: "#9d174d", background: "#fdf2f8", padding: "6px 14px", borderRadius: 20, border: "1px solid #fbcfe8", fontWeight: 700 }}>
                💡 Nhận xét của bạn giúp Spa không ngừng nâng cao chất lượng dịch vụ
              </span>
            </div>

            {loadingHistory ? (
              <p style={{ color: "#64748b", textAlign: "center", padding: 40, fontWeight: 600 }}>⏳ Đang tải nhật ký đánh giá...</p>
            ) : comboHistory.length === 0 ? (
              <div style={{ background: "#fff", borderRadius: 20, padding: 50, textAlign: "center", border: "1.5px dashed #fbcfe8", color: "#64748b" }}>
                <div style={{ fontSize: 40, marginBottom: 10 }}>🌸</div>
                <h3 style={{ color: "#831843", fontSize: 16, margin: "0 0 6px 0" }}>Chưa có liệu trình Combo hoàn thành</h3>
                <p style={{ margin: 0, fontSize: 13 }}>Sau khi hoàn thành các buổi trải nghiệm tại Spa, bạn có thể thực hiện đánh giá tại đây!</p>
              </div>
            ) : (
                  ...
                  <div key={item.AppointmentId} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                          ...
                          <span style={{ fontSize: 12, color: "#64748b", fontWeight: 600 }}>📅 {formatDate(item.AppointmentDate)}</span>
                          {item.StartTime && (
                            <span style={{ fontSize: 12, color: "#db2777", fontWeight: 700, background: "#fdf2f8", padding: "2px 8px", borderRadius: 8, border: "1px solid #fbcfe8" }}>
                              ⏰ {item.StartTime?.slice(0,5)}{item.EndTime ? ` - ${item.EndTime?.slice(0,5)}` : ""}
                            </span>
                        ...
                        <h3 style={{ margin: 0, fontSize: 19, fontWeight: 800, color: "#831843" }}>{item.PackageName}</h3>
                      </div>

                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        {item.IsReviewed ? (
                          <>
                            <span style={{ background: "#f0fdf4", color: "#166534", border: "1px solid #bbf7d0", padding: "6px 14px", borderRadius: 14, fontWeight: 800, fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                              ✅ Đã gửi đánh giá
                            </span>
                            <button
                    ...
                    {item.IsReviewed && (
                      <div style={{
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: item.OverallComment ? 10 : 0, flexWrap: "wrap", gap: 10 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ fontSize: 22 }}>🌟</span>
                            <b style={{ color: "#9f1239", fontSize: 14, textTransform: "uppercase", letterSpacing: 0.5 }}>
                              Đánh Giá Tổng Thể Gói Combo
                            </b>
                          </div>

                          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#ffffff", padding: "6px 14px", borderRadius: 20, border: "1px solid #fda4af", boxShadow: "0 2px 6px rgba(0,0,0,0.03)" }}>
                            <span style={{ color: "#f59e0b", fontSize: 18, fontWeight: 800 }}>
                              {"★".repeat(item.OverallRating || 5)}
                            </span>
                            <b style={{ color: "#9f1239", fontSize: 13 }}>({item.OverallRating || 5}/5 sao)</b>
                          </div>
                        </div>
                        ...
                        {item.OverallComment && (
                          <div style={{
                            ...
                            <div>
                              <b style={{ color: "#be185d", fontSize: 12, display: "block", marginBottom: 2 }}>Nhận xét tổng quan:</b>
                              <span style={{ fontStyle: "italic", color: "#4c0519", fontWeight: 600 }}>"{item.OverallComment}"</span>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      {(item.Services || []).map((s, idx) => (
                        <div key={idx} style={{
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, marginBottom: 8 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <span style={{ background: "#fdf2f8", color: "#db2777", fontWeight: 800, fontSize: 12, width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid #fbcfe8" }}>
                                {idx + 1}
                              </span>
                              <div>
                                <b style={{ color: "#831843", fontSize: 14 }}>{s.ServiceName}</b>
                                <span style={{ fontSize: 11, color: "#64748b", marginLeft: 8 }}>⏱ {s.DurationMinutes || 30} phút</span>
                              </div>
                            </div>

                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#f8fafc", padding: "4px 10px", borderRadius: 20, border: "1px solid #e2e8f0" }}>
                                <img
                                  src={resolveFileUrl(s.TechnicianAvatar) || "/images/avatars/default-avatar.png"}
                                  alt={s.TechnicianName}
                                  style={{ width: 24, height: 24, borderRadius: "50%", objectFit: "cover", border: "1px solid #ec4899" }}
                                />
                                <span style={{ fontSize: 12, fontWeight: 700, color: "#1e293b" }}>KTV: {s.TechnicianName || "Salon KTV"}</span>
                              </div>

                              <div>
                                {s.TechnicianRating > 0 ? (
                                  <span style={{ color: "#f59e0b", fontWeight: 800, fontSize: 14, background: "#fffbeb", padding: "4px 10px", borderRadius: 12, border: "1px solid #fef3c7" }}>
                                    {"★".repeat(s.TechnicianRating)} <small style={{ color: "#92400e", fontWeight: 700 }}>({s.TechnicianRating}/5)</small>
                                  </span>
                                ) : (
                                  <span style={{ fontSize: 11, color: "#94a3b8", fontStyle: "italic" }}>Chưa chấm sao</span>
                                )}
                              </div>
                            </div>
                          </div>
                          ...
                          {s.ServiceComment ? (
                            <div style={{
                              marginTop: 10,
                              padding: "10px 14px",
                              background: "linear-gradient(135deg, #fff1f2 0%, #fff0f6 100%)",
                              borderRadius: 14,
                              border: "1px solid #fecdd3",
                              color: "#881337",
                              fontSize: 13,
                              ...
                              <span style={{ fontSize: 16 }}>💬</span>
                              <div>
                                <b style={{ color: "#be185d", fontSize: 12, display: "block", marginBottom: 2 }}>Nhận xét của bạn:</b>
                                <span style={{ fontStyle: "italic", color: "#4c0519" }}>"{s.ServiceComment}"</span>
                              </div>
                            </div>
                          ) : s.TechnicianRating > 0 ? (
                            <div style={{ marginTop: 6, fontSize: 12, color: "#94a3b8", fontStyle: "italic", paddingLeft: 36 }}>
        ...
        {cancelModalPkgId && (
          <div style={{
            ...
            <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 480, padding: 24, boxShadow: "0 20px 50px rgba(0,0,0,0.25)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, borderBottom: "1px solid #fee2e2", pb: 12 }}>
                <h3 style={{ margin: 0, color: "#be123c", fontSize: 18, fontWeight: 800 }}>❌ Hủy Lịch Hẹn Combo</h3>
                <button onClick={() => setCancelModalPkgId(null)} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer" }}>✕</button>
              </div>

              <div style={{ background: "#fff1f2", border: "1px solid #fecdd3", borderRadius: 12, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#881337" }}>
                <b>ℹ️ LƯU Ý BẢO LƯU GÓI:</b>
                <p style={{ margin: "4px 0 0 0", fontSize: 12, lineHeight: 1.5 }}>
                  Khi hủy lịch hẹn này, <strong>số lượt sử dụng trong gói Combo của bạn vẫn được giữ nguyên 100%</strong>. Bạn có thể tự do đặt lại lịch khác bất kỳ lúc nào rảnh!
                </p>
              </div>

              <label style={{ display: "block", marginBottom: 16 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#1e293b", display: "block", marginBottom: 6 }}>
                ...
                <input
                  ...
                  style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1.5px solid #fda4af", fontSize: 13 }}
                />
              </label>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button
                  ...
                  style={{ flex: 1, padding: 12, borderRadius: 12, border: "1px solid #cbd5e1", background: "#f8fafc", cursor: "pointer", fontWeight: 700, fontSize: 13 }}
                >
                  Bỏ Qua
                </button>
                <button
        ...
        {showPaymentSuccessModal && (
          <Modal open={true} onClose={() => setShowPaymentSuccessModal(false)} title="🎉 THANH TOÁN GÓI COMBO THÀNH CÔNG">
            <div style={{ textAlign: "center", padding: "10px 0" }}>
              <div style={{
                width: 64, height: 64,
                ...
                fontSize: 32, margin: "0 auto 12px auto",
              ...
              <h3 style={{ color: "#15803d", margin: "0 0 4px 0", fontSize: 22, fontWeight: 800 }}>
                THANH TOÁN THÀNH CÔNG!
              </h3>
              <p style={{ fontSize: 13, color: "#475569", margin: "0 0 16px 0" }}>
                Cảm ơn quý khách đã đăng ký Gói Combo Spa trọn gói tại Luna Salon & Spa.
              </p>

              <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 16, padding: 18, textAlign: "left", marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, paddingBottom: 10, borderBottom: "1px dashed #cbd5e1" }}>
                  <span style={{ fontSize: 24 }}>📦</span>
                  <div>
                    <span style={{ background: "#166534", color: "#fff", fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 10 }}>GÓI COMBO TRỌN GÓI</span>
                    <h4 style={{ margin: "2px 0 0 0", color: "#14532d", fontSize: 16, fontWeight: 800 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, fontSize: 13 }}>
                  <div>
                    <span style={{ color: "#64748b", fontSize: 11, fontWeight: 700 }}>MÃ GÓI COMBO:</span>
                    <div style={{ fontWeight: 800, color: "#1e293b" }}>#{selectedPkg?.CustomerPackageId || "COMBO"}</div>
                  </div>
                  <div>
                    <span style={{ color: "#64748b", fontSize: 11, fontWeight: 700 }}>TRẠNG THÁI:</span>
                    <div style={{ fontWeight: 800, color: "#15803d" }}>✓ Đã thanh toán 100%</div>
                  </div>
                </div>

                {selectedPkg?.ServiceNames && (
                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px dashed #cbd5e1" }}>
                    <span style={{ color: "#166534", fontSize: 11, fontWeight: 800, display: "block", marginBottom: 4 }}>DỊCH VỤ TRỌN GÓI ĐI KÈM:</span>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {String(selectedPkg.ServiceNames).split(",").map((svc, i) => (
                        <span key={i} style={{ background: "#ffffff", color: "#166534", border: "1px solid #86efac", padding: "3px 8px", borderRadius: 8, fontSize: 11, fontWeight: 600 }}>
                          ✓ {svc.trim()}
                        </span>
                      ))}
                    </div>
