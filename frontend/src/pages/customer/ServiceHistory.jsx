      ...
      <div className="service-history-pro">
        ...
        <section className="service-history-stats">
          ...
          <article>
            ...
            <div className="stat-card-content">
              <p>Tổng chi tiêu</p>
              <strong>{loading ? "..." : formatMoney(stats.totalSpent)}</strong>
          ...
          <article>
            ...
            <div className="stat-card-content">
              ...
              <strong>
                {loading ? "..." : `${filteredStats.duration} phút`}
              </strong>
...
function HistoryCard({
  ...
  const displayTitle = isCombo
    ? item.PackageName || item.ServiceName || "Gói Combo Spa"
    : item.ServiceName || "Dịch vụ";
  const displayImage = isCombo
    ? item.PackageImageUrl || item.ServiceImageUrl
  ...
  const allTechNames =
    isCombo && Array.isArray(item.services)
      ? Array.from(
          new Set(
            item.services
              .map((s) => s.StepTechnicianName || s.EmployeeName)
              .filter(Boolean),
          ),
        ).join(", ")
  ...
  const comboTechnicians = useMemo(() => {
    ...
    for (const s of item.services) {
      ...
      const techAvatar =
        s.StepTechnicianAvatar || s.EmployeeImageUrl || FALLBACK_AVATAR;
  ...
  return (
    <article className="history-service-card-pro">
      <div className="history-service-image">
        ...
        />
        <span>
          {isCombo ? "Gói Combo Spa" : item.CategoryName || "Beauty Service"}
        </span>
      </div>
      ...
      <div className="history-service-body">
        <div className="history-card-head">
          ...
          <div className="history-card-price-group">
            <b className="history-card-price">
              {isCombo ? "Trọn gói Combo" : formatMoney(displayPrice)}
            </b>
            {isPackagePaid && (
            ...
            >
              ...
              >
                {item.services.map((s, idx) => {
                  const stepTimeStr = getStepTimeRange(
                    s,
                    item.services,
                    item.StartTime,
                  );
                    ...
                    >
                      <div>
                        ...
                        <span style={{ fontSize: 11, color: "#64748b" }}>
                          ⏱{" "}
                          {stepTimeStr
                            ? `${stepTimeStr} (${s.DurationMinutes || 30} phút)`
        ...
        <div className="history-info-grid">
          ...
          <div className="info-item">
            ...
            <div className="info-text">
              ...
              <span
                className={`payment-status-badge ${isCombo ? "badge-paid" : paymentBadgeClass(item.PaymentStatus)}`}
              >
          ...
          <div className="info-item">
            ...
            <div className="info-text">
              ...
              <b className="voucher-code-text">
                {item.PackageName || item.VoucherCode || "Gói Combo"}
              </b>
            </div>
          </div>
        </div>
            ...
            {comboTechnicians.map((tech, tIdx) => (
              ...
              >
                ...
                <div className="employee-info-content" style={{ flex: 1 }}>
                  <b style={{ fontSize: 13, color: "#831843" }}>
                    {tech.EmployeeName}
                  </b>
                  <span style={{ fontSize: 11, color: "#64748b" }}>
                    {tech.Specialization || "Chuyên viên làm đẹp"}
                  </span>
                </div>
                {tech.EmployeeId && (
                  <button
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill={
                        Number(tech.IsFavoriteEmployee)
                          ? "currentColor"
                          : "none"
                      }
                      stroke="currentColor"
          ...
          <div className="history-employee-mini">
            ...
            >
              <svg
                ...
                height="20"
                viewBox="0 0 24 24"
                fill={Number(item.IsFavoriteEmployee) ? "currentColor" : "none"}
                stroke="currentColor"
                strokeWidth="2.5"
              >
                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"></path>
          ...
          {reviewed ? (
            <>
              <div className="history-stars">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg
                    ...
                    stroke={
                      i < Number(item.Rating || 0) ? "#ffb800" : "#d0d0d0"
                    }
                    strokeWidth="2.5"
              ...
              <p className="review-comment-text">
                "{item.ReviewComment || "Không có nội dung đánh giá."}"
              </p>
            ...
            <div className="history-not-reviewed">
              <b>
                {isCombo
                  ? "Chưa đánh giá ca hẹn Combo này"
                  : "Chưa đánh giá dịch vụ này"}
              </b>
              <span>
                Hãy đánh giá để giúp salon cải thiện chất lượng dịch vụ tốt hơn.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="history-actions-pro">
          {isCombo ? (
            ...
            >
              <button
                ...
                onClick={() =>
                  navigate(
                    `/customer/packages?packageId=${item.CustomerPackageId}`,
                  )
                }
          ...
          >
            <svg
              ...
              height="14"
              viewBox="0 0 24 24"
              fill={Number(item.IsFavoriteService) ? "currentColor" : "none"}
              stroke="currentColor"
              strokeWidth="2.5"
            >
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"></path>
