...
export default function WaitingListPage() {
  ...
  async function handleConfirmMatch(waitingId) {
    try {
      ...
      setMessage(
        "Xác nhận lịch ghép thành công! Đang chuyển hướng sang thanh toán...",
      );
      const appointmentId =
        res.data?.data?.appointmentId || res.data?.data?.AppointmentId;
      if (appointmentId) {
        setTimeout(() => {
  ...
  async function handleRejectMatch(waitingId) {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn từ chối khung giờ được ghép này? Yêu cầu hàng chờ của bạn sẽ bị hủy.",
      )
    ) {
      return;
    }
    try {
      ...
      setMessage(
        "Đã từ chối lịch ghép thành công. Hệ thống đang tìm người thay thế.",
      );
      await loadItems();
    } catch (err) {
  ...
  const selectedBranchObject = useMemo(() => {
    return branches.find(
      (b) => String(b.BranchId) === String(form.preferredBranchId),
    );
  ...
  const availableEmployees = useMemo(() => {
    ...
    if (form.preferredBranchId) {
      list = list.filter(
        (e) => String(e.BranchId) === String(form.preferredBranchId),
      );
    }
    return list;
  ...
  useEffect(() => {
    ...
    if (serviceId || employeeId || date || acceptTech || acceptTime) {
      setForm((prev) => {
        const isAcceptTech =
          acceptTech === "true" || acceptTech === "1"
            ? true
            : prev.acceptOtherTechnician;
        const isAcceptTime =
          acceptTime === "true" || acceptTime === "1"
            ? true
            : prev.acceptOtherTimeSlots;
        return {
          ...prev,
          serviceId: serviceId || prev.serviceId,
          preferredEmployeeId: isAcceptTech
            ? ""
            : employeeId || prev.preferredEmployeeId,
  ...
  useEffect(() => {
    if (form.preferredEmployeeId && employees.length > 0) {
      ...
      if (
        selectedEmpObj &&
        selectedEmpObj.BranchId &&
        String(form.preferredBranchId) !== String(selectedEmpObj.BranchId)
      ) {
        setForm((prev) => ({
          ...prev,
  ...
  async function submit(e) {
    ...
    if (form.preferredDate && form.preferredDate < minDate) {
      setError(
        "Thời gian hoạt động trong ngày đã kết thúc hoặc ngày chọn ở quá khứ. Vui lòng chọn ngày khác.",
      );
      return;
    }
      ...
      <div className="wlx-page">
        ...
        <section className="wlx-create-grid">
          {!form.waitingId && !searchParams.get("serviceId") ? (
            <div
              className="wlx-form wlx-info-placeholder"
              style={{
                padding: "40px",
                textAlign: "center",
            ...
            >
              ...
              <h2 style={{ color: "#ef4f83", marginBottom: "15px" }}>
                Không thể tạo yêu cầu trực tiếp
              </h2>
              <p
                style={{
                  fontSize: "15px",
              ...
              >
                Yêu cầu xếp hàng chờ chỉ được thực hiện khi bạn{" "}
                <strong>
                  đặt lịch hẹn bình thường nhưng hết slot trống phù hợp
                </strong>
                .
              </p>
              <p
              ...
              >
                Vui lòng truy cập trang Đặt lịch hẹn để chọn dịch vụ và thời
                gian mong muốn.
              </p>
              <button
                type="button"
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const currentEmp = employees.find(
                          (x) =>
                            String(x.EmployeeId) ===
                            String(form.preferredEmployeeId),
                        );
                        const nextEmpId =
                          currentEmp &&
                          nextBranchId &&
                          String(currentEmp.BranchId) !== String(nextBranchId)
                            ? ""
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setForm({
                          ...
                          preferredBranchId: empObj
                            ? String(empObj.BranchId)
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            preferredTimeFrom: e.target.value,
                          })
                        }
                      />
                    </label>
          ...
          <aside className="wlx-side-panel">
            ...
            <div>
              <b>1</b>
              <p>
                <strong>Gửi yêu cầu</strong>Chọn dịch vụ, thời gian, KTV và mức
                ưu tiên.
              </p>
            </div>
            <div>
              <b>2</b>
              <p>
                <strong>Hệ thống tìm kiếm</strong>Hệ thống tự động ghép slot
                trống phù hợp với yêu cầu của bạn.
              </p>
            </div>
            <div>
              <b>3</b>
              <p>
                <strong>⚡ Khớp slot</strong>Khi tìm được slot, hệ thống giữ chỗ
                và thông báo cho bạn xác nhận.
              </p>
            </div>
            <div>
              <b>4</b>
              <p>
                <strong>Xác nhận &amp; Thanh toán</strong>Bạn xác nhận slot được
                ghép và tiến hành thanh toán. Nếu từ chối, yêu cầu sẽ bị hủy.
              </p>
            </div>
            <div>
              <b>5</b>
              <p>
                <strong>✅ Hoàn tất</strong>Lịch hẹn chính thức được tạo và chờ
                đến ngày phục vụ.
              </p>
            </div>
          </aside>
                            ...
                            <b>
                              <a
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  navigate(
                                    `/customer/appointments/${item.ConvertedAppointmentId}`,
                                  );
                                }}
                                style={{
                                  color: "#db2777",
                              ...
                              >
                                Xem lịch hẹn AP
                                {String(item.ConvertedAppointmentId).padStart(
                                  5,
                                  "0",
                                )}{" "}
                                📅
                      ...
                      <div className="wlx-matched-banner">
                        <div className="wlx-matched-header">
                          <svg
                            ...
                            stroke="currentColor"
                            strokeWidth="2"
                            style={{ marginRight: "8px", color: "#db2777" }}
                          >
                        ...
                        <div className="wlx-matched-body">
                          ...
                          <div className="wlx-matched-row">
                            <span>Khung giờ:</span>
                            <strong>
                              {timeText(item.MatchedStartTime)} -{" "}
                              {timeText(item.MatchedEndTime)}
                            </strong>
                          </div>
                          <div className="wlx-matched-row">
                            <span>Ngày hẹn:</span>
                ...
                {detail.Status === "MATCHED" && (
                  <>
                    ...
                    <div>
                      ...
                      <b>
                        {timeText(detail.MatchedStartTime)} -{" "}
                        {timeText(detail.MatchedEndTime)}
                      </b>
                    </div>
                    <div>
                      ...
                      <b>
                        <a
                          ...
                          onClick={(e) => {
                            ...
                            setDetail(null);
                            navigate(
                              `/customer/appointments/${detail.ConvertedAppointmentId}`,
                            );
                          }}
                          style={{
                            color: "#db2777",
                        ...
                        >
                          Xem lịch hẹn AP
                          {String(detail.ConvertedAppointmentId).padStart(
                            5,
                            "0",
                          )}{" "}
                          📅
