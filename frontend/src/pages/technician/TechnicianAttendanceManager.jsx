...
export default function TechnicianAttendanceManager() {
  ...
  const loadData = async () => {
    try {
      ...
      console.error(err);
      setError(
        err.response?.data?.message || "Không tải được dữ liệu hệ thống",
      );
    } finally {
      setLoading(false);
    }
      ...
      const res = await axiosClient.post("/technician/shifts/register", {
        ...
        serviceIds:
          selectedServiceIds.length > 0
            ? selectedServiceIds
            : capableServices.map((s) => s.ServiceId),
      });
  ...
  const handleCancelRegistration = async (shiftId) => {
    ...
    try {
      ...
      const res = await axiosClient.delete(
        `/technician/shifts/register/${shiftId}`,
      );
  ...
  const filteredRegDateShifts = useMemo(() => {
    ...
    return availableShifts.filter(
      (s) => formatDateLocal(s.ShiftDate) === selectedDateStr,
    );
      ...
      >
        ...
        >
          <div>
            ...
            >
              Đăng ký ca trực linh hoạt và quản lý phân công làm việc
            </p>
          </div>
          <div
            style={{
        ...
        >
          ...
          >
            ...
            >
              {weeklyStats.registeredCount || 0} ca
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              {Number(weeklyStats.estimatedEarnings || 0).toLocaleString()}đ
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <button
                onClick={() => setActiveSubTab("my-shifts")}
                style={{
                  border: "none",
                  outline: "none",
                  backgroundColor: "transparent",
                ...
                >
                  ...
                  >
                    <b style={{ fontSize: "0.85rem" }}>
                      Tháng {String(regDate.getMonth() + 1).padStart(2, "0")},{" "}
                      {regDate.getFullYear()}
                    </b>
                  ...
                  >
                    ...
                    {miniCalendarDays.map((day, idx) => {
                      ...
                      const isSelected =
                        day.toDateString() === regDate.toDateString();
                      const dayStr = formatDateLocal(day);
                      // Check if any open shifts exist for this day
                      const hasOpenShift = availableShifts.some(
                        (s) => formatDateLocal(s.ShiftDate) === dayStr,
                      );
                        ...
                        return (
                          ...
                          >
                            ...
                            {s.IsRegistered ? (
                              ...
                              <button
                                onClick={() => triggerOpenRegisterModal(s)}
                  ...
                  <tbody>
                    {myShifts.length === 0 ? (
                      ...
                      myShifts.map((s, idx) => {
                        ...
                        const isPast =
                          shiftDate < new Date().setHours(0, 0, 0, 0);
                        return (
                          <tr
                            key={idx}
                          ...
                          >
                            ...
                            <td style={{ padding: "8px", textAlign: "right" }}>
                              {!isPast && (
                                <button
                                  onClick={() =>
                                    handleCancelRegistration(s.ShiftId)
                                  }
                                  disabled={actionLoading}
                                  style={{
                                    backgroundColor: "transparent",
            ...
            >
              ...
              >
                ...
                >
                  ⏰ Thời gian:{" "}
                  <b>
                    {selectedShiftForReg.StartTime} -{" "}
                    {selectedShiftForReg.EndTime}
                  </b>
                </div>
                <div style={{ fontSize: "0.85rem", color: "#4a5568" }}>
                  📆 Ngày trực:{" "}
                  <b>
                    {new Date(selectedShiftForReg.ShiftDate).toLocaleDateString(
                      "vi-VN",
                    )}
                  </b>
                </div>
