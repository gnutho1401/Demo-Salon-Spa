...
export default function ReceptionistPackages() {
  ...
  const loadComboAppointments = async () => {
    ...
    try {
      ...
      const filtered = Array.isArray(list)
        ? list.filter((a) => a.CustomerPackageId || a.CustomerPackageName)
        : [];
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await axiosClient.get(
          `/receptionist/customers?search=${encodeURIComponent(custSearch.trim())}`,
        );
  ...
  const loadCustomerPackages = async (customerId) => {
    ...
    try {
      const res = await axiosClient.get(
        `/packages/my?customerId=${customerId}`,
      );
  ...
  const handleSellPackage = async () => {
    ...
    try {
      ...
      setMessage(
        `✅ Bán gói Combo "${sellModalPkg.PackageName}" cho khách hàng ${selectedCust.FullName} thành công!`,
      );
  ...
  const handleBookComboForCustomer = async () => {
    ...
    try {
      const res = await axiosClient.post(
        `/packages/my/${bookModalPkg.CustomerPackageId}/book`,
        {
          ...bookingForm,
          customerId: selectedCust?.CustomerId,
        },
      ...
      setMessage(
        `✅ Đặt lịch Combo thành công cho khách hàng ${selectedCust?.FullName || ""}`,
      );
      if (selectedCust) loadCustomerPackages(selectedCust.CustomerId);
      loadComboAppointments();
    } catch (err) {
      setBookingError(
        err.response?.data?.message || "Lỗi đặt lịch Combo giúp khách",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleCheckIn = async (appointmentId) => {
    try {
      ...
      setMessage(
        `✅ Đã Check-in đón khách làm Combo #${appointmentId}. Các bước dịch vụ đã được mở khóa theo thứ tự!`,
      );
      loadComboAppointments();
    } catch (err) {
  ...
  const handleComplete = async (appointmentId) => {
    try {
      await axiosClient.put(
        `/receptionist/appointments/${appointmentId}/complete`,
      );
  ...
  const handleCancel = async (appointmentId) => {
    if (
      !window.confirm(
        `Hủy cuộc hẹn Combo #${appointmentId}? Số lượt sử dụng sẽ được hoàn trả lại cho khách.`,
      )
    )
      return;
    try {
      await axiosClient.put(
        `/receptionist/appointments/${appointmentId}/cancel`,
        { cancelReason: "Lễ tân hủy giúp khách" },
      );
  ...
  const handleMarkNoShow = async (appointmentId) => {
    if (
      !window.confirm(
        `Xác nhận đánh dấu VẮNG MẶT (No-Show) cho cuộc hẹn Combo #${appointmentId}? Hệ thống sẽ gửi email & thông báo web cho khách hàng!`,
      )
    )
      return;
    try {
      ...
      setMessage(
        `✅ Đã đánh dấu VẮNG MẶT cho cuộc hẹn Combo #${appointmentId}. Đã tự động gửi thông báo qua Web & Email cho khách hàng!`,
      );
      loadComboAppointments();
    } catch (err) {
  ...
  const handleUpdateStepStatus = async (appointmentServiceId, newStatus) => {
    try {
      const res = await axiosClient.put(
        `/receptionist/appointment-services/${appointmentServiceId}/status`,
        { status: newStatus },
      );
      if (res.data?.data?.allCompleted) {
        setMessage(
          "🎉 Tất cả các bước dịch vụ đã hoàn thành! Cuộc hẹn đã tự động hoàn thành.",
        );
      } else {
        setMessage(
          "✅ Đã xác nhận xong bước dịch vụ! Bước tiếp theo đã mở khóa.",
        );
      }
      loadComboAppointments();
    } catch (err) {
  ...
  const displayedComboAppts = comboAppts.filter((a) => {
    const matchStatus =
      ...
      (a.Status || "").toUpperCase() === apptStatusFilter;
    const matchKeyword =
      !apptSearch ||
      ...
      <div style={{ padding: "24px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <h1 style={{ margin: "4px 0 0 0", fontSize: 24, fontWeight: 800 }}>
              Quản Lý & Duyệt Đặt Lịch Gói Combo
            </h1>
            <p style={{ margin: "4px 0 0 0", fontSize: 13, opacity: 0.9 }}>
              Duyệt lịch hẹn Combo, đón khách check-in tại quầy, quản lý KTV
              rảnh & bán gói tại quầy
            </p>
          </div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <button
              style={{
                ...
                cursor: "pointer",
                background:
                  activeTab === "shop" ? "#ffffff" : "rgba(255,255,255,0.18)",
                color: activeTab === "shop" ? "#0d9488" : "#ffffff",
                boxShadow:
                  activeTab === "shop" ? "0 4px 12px rgba(0,0,0,0.15)" : "none",
                transition: "all 0.2s",
              }}
          ...
          <div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: "1px solid",
                        borderColor:
                          apptStatusFilter === tab.key ? "#0d9488" : "#cbd5e1",
                        background:
                          apptStatusFilter === tab.key ? "#0d9488" : "#f8fafc",
                        color:
                          apptStatusFilter === tab.key ? "#ffffff" : "#475569",
                        fontSize: 12,
                        fontWeight: 700,
                        cursor: "pointer",
                      }}
            ...
            {loadingComboAppts ? (
              <p style={{ color: "#64748b", textAlign: "center", padding: 30 }}>
                Đang tải danh sách lịch hẹn Combo...
              </p>
              ...
              >
                {displayedComboAppts.map((a) => {
                  const servicesList =
                    typeof a.ServicesJson === "string"
                      ? JSON.parse(a.ServicesJson)
                      : a.ServicesJson || [];
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 300 }}>
                          ...
                          >
                            <img
                              src={
                                resolveFileUrl(a.CustomerAvatarUrl) ||
                        ...
                        >
                          ...
                          >
                            ⏰ Khung giờ: {a.StartTime?.slice(0, 5)} -{" "}
                            {a.EndTime?.slice(0, 5)}
                          </div>
                          <div
                            style={{
                          ...
                          {a.Status !== "COMPLETED" &&
                            ...
                            a.Status !== "NO_SHOW" && (
                              <button
                                ...
                                title={
                                  !isCheckInDone
                                    ? "Cần Check-in đón khách trước khi hoàn thành"
                                    : ""
                                }
                                style={{
                                  padding: "8px 16px",
                      ...
                      {servicesList.length > 0 && (
                        ...
                        >
                          ...
                          >
                            <span>
                              ✂️ Danh sách bước dịch vụ Combo (
                              {
                                servicesList.filter(
                                  (s) => s.Status === "COMPLETED",
                                ).length
                              }
                              /{servicesList.length} bước xong)
                            </span>
                            {a.Status === "NO_SHOW" ? (
                              <span
                                style={{
                              ...
                              >
                                🚨 Khách vắng mặt (No-Show) - Đã khóa toàn bộ
                                dịch vụ
                              </span>
                            ) : a.Status === "CANCELLED" ? (
                              <span
                              ...
                              !isCheckInDone && (
                                ...
                                >
                                  🔒 Cần bấm "Check-in (Đón Khách)" để mở khóa
                                  làm dịch vụ
                                </span>
                              )
                            )}
                          ...
                          >
                            {servicesList.map((svcStep, idx) => {
                              const prevStepsCompleted = servicesList
                                .slice(0, idx)
                              ...
                              const isUnlocked =
                                ...
                                !isNoShowOrCancelled;
                              const isCompleted =
                                svcStep.Status === "COMPLETED";
                              const isInProgress =
                                svcStep.Status === "IN_PROGRESS";
            ...
            >
              ...
              {selectedCust ? (
                ...
                >
                  ...
                  <span style={{ fontSize: 12, color: "#115e59" }}>
                    ({selectedCust.Phone || selectedCust.Email})
                  </span>
                  <button
                    onClick={() => {
                      ...
                      {custResults.map((c) => (
                        ...
                        >
                          ...
                          <span style={{ fontSize: 11, color: "#64748b" }}>
                            SĐT: {c.Phone || "N/A"} | Email: {c.Email || "N/A"}
                          </span>
                        </div>
                      ))}
                    </div>
                      ...
                      <div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (!selectedCust) {
                              alert(
                                "Vui lòng chọn khách hàng ở thanh tìm kiếm bên trên trước khi bán gói!",
                              );
                              return;
                            }
                            setSellModalPkg(p);
            ...
            {!selectedCust ? (
              ...
              >
                ...
                <p style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>
                  Vui lòng nhập Tên/SĐT khách hàng ở ô tìm kiếm phía trên để xem
                  các Combo của khách & hỗ trợ đặt lịch!
                </p>
              </div>
            ) : (
              <div>
                ...
                {loadingCustPackages ? (
                  ...
                  >
                    <p style={{ color: "#64748b", margin: 0 }}>
                      Khách hàng này chưa có gói Combo nào.
                    </p>
                  </div>
                ) : (
                  <div
                    ...
                    {customerPackages.map((cp) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            style={{
                              ...
                              fontWeight: 700,
                              background:
                                cp.Status === "ACTIVE" ? "#f0fdf4" : "#fef2f2",
                              color:
                                cp.Status === "ACTIVE" ? "#166534" : "#991b1b",
                            }}
                          >
                            {cp.Status}
                          </span>
                        ...
                        >
                          Gồm:{" "}
                          <strong>
                            {cp.ServiceNames || "Các dịch vụ spa"}
                          </strong>
                        </p>
                        ...
                        >
                          <span>
                            Lượt dùng:{" "}
                            <b>
                              {cp.UsedSessions || 0}/{cp.TotalSessions || 1}
                            </b>
                          </span>
                          <span>
                            Còn lại:{" "}
                            <b style={{ color: "#0d9488" }}>
                              {cp.RemainingSessions || 0} lượt
                            </b>
                          </span>
                        </div>

                        {cp.Status === "ACTIVE" &&
                          Number(cp.RemainingSessions || 0) > 0 && (
                            <button
                              style={{
                                width: "100%",
                                padding: "10px",
        ...
        {sellModalPkg && (
          ...
          >
            ...
            >
              <h3
                style={{ margin: "0 0 12px 0", color: "#0f766e", fontSize: 18 }}
              >
                💳 Xác Nhận Bán Combo Tại Quầy
              </h3>
              ...
              >
                <b style={{ color: "#166534", fontSize: 14 }}>
                  Khách hàng: {selectedCust?.FullName}
                </b>
                <p
                  style={{
                    margin: "2px 0 0 0",
                ...
                >
                  SĐT: {selectedCust?.Phone || selectedCust?.Email}
                </p>
              </div>
            ...
            >
              ...
              {bookingSuccess ? (
                <div style={{ textAlign: "center", padding: "12px 0" }}>
                  ...
                  >
                    Đơn đặt lịch đã được khởi tạo và tự động gán KTV rảnh cho
                    khách hàng!
                  </p>
                  ...
                  >
                    <div>
                      <b>KTV Được Tự Động Gán:</b>{" "}
                      {bookingSuccess.technician?.fullName} (SĐT:{" "}
                      {bookingSuccess.technician?.phone || "N/A"})
                    </div>
                    <div style={{ marginTop: 4 }}>
                      <b>Khung Giờ:</b> {bookingSuccess.appointmentDate} (
                ...
                <div>
                  ...
                  >
                    ℹ️ Hệ thống sẽ tự động xếp KTV rảnh nhất trong ca làm việc
                    để phục vụ cuộc hẹn Combo này.
                  </div>
