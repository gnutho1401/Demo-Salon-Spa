    ...
    <section className="combo-detail-page">
      <div className="combo-detail-card">
        <div className="combo-detail-img">
          ...
          <img src={resolveFileUrl(item.ImageUrl || '/images/services/default-service.png')} alt={item.PackageName} />
        </div>
        <div className="combo-detail-info">
          ...
          <div className="payment-box">
            ...
            <div className="payment-method-grid">
              ...
              >
                <span className="payment-method-card-icon">📲</span>
                <span className="payment-method-card-title">Thanh toán PayOS</span>
              </div>
            ...
            >
              {paying ? 'Đang xử lý...' : method === 'VNPAY' ? 'Thanh toán qua VNPay' : method === 'PAYOS' ? 'Thanh toán qua PayOS' : 'Xác nhận mua liệu trình'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="combo-services-section">
        ...
        <div style={{ 
          ...
          <h3 style={{ margin: '0 0 10px 0', fontSize: '1.05rem', color: '#2d1424', fontWeight: 800 }}>
            ...
            {(item.Services || []).map(s => {
              ...
              return (
                <li key={s.ServiceId} style={{ marginBottom: '6px' }}>
                  <b>{s.ServiceName}</b>: {money(svcPrice)}/buổi × {count} buổi = <b style={{ color: '#2d1424' }}>{money(lineVal)}</b>
                </li>
              );
            })}
          </ul>
          <div style={{ 
            ...
            <div>
              <span>Tổng giá trị mua lẻ: <del style={{ color: '#8c7784', fontWeight: 600 }}>{money(item.Price)}</del></span>
              <span style={{ marginLeft: '16px', color: '#ef4f83', fontWeight: 800 }}>
                Tiết kiệm ngay: {money(Number(item.Price || 0) - Number(item.FinalPrice || 0))} (-{item.DiscountPercent}%)
              </span>
            </div>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#2d1424' }}>
              Giá trọn gói: <span style={{ color: '#ef4f83', fontSize: '1.3rem' }}>{money(item.FinalPrice)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="combo-service-list">
          {(item.Services || []).map((s) => (
            <Link to={`/services/${s.ServiceId}`} className="combo-service-item" key={s.ServiceId} style={{ background: '#fff', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
              <img src={resolveFileUrl(s.ImageUrl || '/images/services/default-service.png')} alt={s.ServiceName} style={{ width: '120px', height: '120px', objectFit: 'cover' }} />
              <div>
                ...
                <span style={{ fontSize: '0.8rem', color: '#ef4f83', fontWeight: 700 }}>
                  🕘 {s.DurationMinutes || 60} phút • Số lượng: {s.SessionCount || s.MaxSessions || 1} buổi (Đơn giá: {money(s.Price)})
                </span>
              </div>
            </Link>
          ))}
        </div>
        <Link className="card-btn" to="/packages" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', width: 'auto', padding: '10px 20px', marginTop: '20px' }}>
