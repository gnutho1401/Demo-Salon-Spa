...
export default function AdminReports() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await axiosClient.get("/admin/reports/summary", {
        params: filters,
      });
  ...
  const handleReset = () => {
    ...
    setLoading(true);
    axiosClient
      .get("/admin/reports/summary", { params: defaultFilters })
      .then((res) => {
        setData(res.data.data || res.data || null);
        setTimeout(scrollToAppointments, 150);
      })
  ...
  const exportToCSV = () => {
    if (
      ...
      data.customerAppointments.length === 0
    ) {
  ...
  const paginatedAppointments = useMemo(() => {
    ...
    return data.customerAppointments.slice(
      startIndex,
  ...
  const chartPath = useMemo(() => {
    ...
    const points = trend.map((item, idx) => {
      const x =
        padding + (idx * (width - padding * 2)) / Math.max(trend.length - 1, 1);
      const y =
        height - padding - (item.Revenue * (height - padding * 2)) / maxVal;
      return `${x},${y}`;
    });
  ...
  const chartAreaPath = useMemo(() => {
    ...
    const endX =
      padding +
      ((trend.length - 1) * (width - padding * 2)) /
  ...
  const totalServicesRevenue = useMemo(() => {
    ...
    return (
      data.serviceDistribution.reduce((sum, item) => sum + item.Revenue, 0) || 1
    );
        ...
        <div className="filter-grid">
          ...
          <div className="filter-item">
            ...
            >
              ...
              {memberships.map((m) => (
                <option key={m.MembershipLevelId} value={m.MembershipLevelId}>
                  {m.LevelName}
                </option>
              ))}
            </select>
        ...
        <>
          ...
          <div className="metrics-grid">
            <article className="metric-card metric-revenue">
              ...
              <h3>
                {Number(data.revenue?.TotalRevenue || 0).toLocaleString(
                  "vi-VN",
                )}
                đ
              </h3>
            ...
            <article className="metric-card metric-refunds">
              ...
              <p>
                Tổng hoàn:{" "}
                {Number(data.revenue?.TotalRefunded || 0).toLocaleString(
                  "vi-VN",
                )}
                đ
              </p>
            ...
            <article className="metric-card metric-appointments">
              ...
              <p>
                Xong: {data.appointments?.CompletedAppointments || 0} • Hủy:{" "}
                {data.appointments?.CancelledAppointments || 0}
              </p>
            </article>
          </div>
          ...
          <div className="charts-grid">
            <div className="chart-card">
                  ...
                  <svg className="svg-chart" viewBox="0 0 600 180">
                    ...
                    {data.dailyTrend.map((item, idx) => {
                      ...
                      const x =
                        padding +
                        (idx * (width - padding * 2)) /
                          Math.max(data.dailyTrend.length - 1, 1);
                      const y =
                        height -
                        padding -
                        (item.Revenue * (height - padding * 2)) / maxVal;
                      return (
                        <circle
                          key={idx}
                          cx={x}
              ...
              >
                <span>
                  {data.dailyTrend && data.dailyTrend[0]
                    ? formatDateString(data.dailyTrend[0].RevenueDate)
                    : ""}
                </span>
                <span>
                  Doanh thu cao nhất:{" "}
                ...
                <span>
                  ...
                  data.dailyTrend[data.dailyTrend.length - 1]
                    ? formatDateString(
                        data.dailyTrend[data.dailyTrend.length - 1].RevenueDate,
                      )
                    : ""}
                </span>
              </div>
              ...
              >
                ...
                data.serviceDistribution.length > 0 ? (
                  data.serviceDistribution.map((item) => {
                    const pct = (
                      (item.Revenue / totalServicesRevenue) *
                      100
                    ).toFixed(1);
                    return (
          ...
          <div className="tables-grid">
            <div className="table-card">
              <h3 className="table-title">
                Top 10 dịch vụ được yêu thích nhất
              </h3>
                    ...
                    {(data.topServices || []).map((item) => (
                      <tr key={item.ServiceId}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          {item.AppointmentCount} lượt
                        </td>
                        <td>
                          {Number(item.ServiceRevenue || 0).toLocaleString(
                            "vi-VN",
                          )}
                          đ
                        </td>
            ...
            <div className="table-card">
              <h3 className="table-title">
                Top 10 nhân viên phục vụ nhiều nhất
              </h3>
              <div style={{ overflowX: "auto" }}>
                <table className="simple-table">
                  <thead>
          ...
          <div
            ...
            id="reports-appointments-section"
                  ...
                  {paginatedAppointments.map((item) => (
                    <tr key={item.AppointmentId}>
                      ...
                      <td style={{ fontWeight: 700 }}>
                        {Number(item.FinalAmount || 0).toLocaleString("vi-VN")}đ
                      </td>
                      <td>
                        <span
                          className={`status-pill status-${(item.PaymentStatus || "pending").toLowerCase()}`}
                        >
                      ...
                      <td>
                        <span
                          className={`status-pill status-${(item.AppointmentStatus || "pending").toLowerCase()}`}
                        >
            ...
            {totalPages > 1 && (
              <div className="pagination-container">
                <span className="pagination-info">
                  Hiển thị trang {currentPage} / {totalPages} (Tổng{" "}
                  {data.customerAppointments.length} lịch hẹn)
                </span>
